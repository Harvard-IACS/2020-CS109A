...
for i in trange(len(exp_vals)):
    ...
    score_train[exp_vals[i]] = accuracy_score(y_train, model.predict(x_train))
    score_test[exp_vals[i]] = accuracy_score(y_test, model.predict(x_test))
    
    staged_train_scores[exp_vals[i]] = list(model.staged_score(x_train,y_train))
    staged_test_scores[exp_vals[i]] = list(model.staged_score(x_test, y_test))
